...
RSpec.describe RepositoriesController, :type => :controller do
  ...
  let(:repo_issue1) { double({assignee: nil, html_url: "www.github.com/user/repo/issues/40", user: double({login: "sophie"}), state: "open", title: "Issue With A Thing", body: "blah blah", created_at: Date.today}) }
  let(:repo_issue2) { double({assignee: nil, html_url: "www.github.com/user/repo/issues/40", user: double({login: "sophie"}), state: "open", title: "Issue With A Different Thing", body: "yadda yadda", created_at: Date.yesterday}) }
  ...
  describe "#create" do
    before(:each) do
      allow(GithubAdapter).to receive(:client).and_return(client)
